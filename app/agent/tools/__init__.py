...
from app.agent.tools.rag_tools import ListReadBooksTool, SearchBookTool
...
def build_registry(rag, store, text_reader, client, document_id, max_read_page):
    ...
    if rag is not None:  # RAG 不可用时退化为只有阅读记录工具
        registry.add(SearchBookTool(rag, store, document_id, max_read_page))
        registry.add(ListReadBooksTool(store))
