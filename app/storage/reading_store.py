...
class ReadingStore:
    ...
    Pure data layer — Qt-free. Accessed from both the main thread (writes on
    dwell) and worker threads (agent reading tools), so the connection allows
    cross-thread use and a lock serializes every operation.
    """
        ...
        with self._lock:
            self._conn.execute(
                ...
                ON CONFLICT(document_id, page_number) DO UPDATE SET
                    last_read_at = datetime('now', 'localtime'),
    ...
    def reading_progress(self, document_id):
        """How much of this book has been read: pages_read + max_read_page."""
        with self._lock:
            row = self._conn.execute(
                "SELECT COUNT(*), MAX(page_number) FROM page_reads WHERE document_id = ?",
                (document_id,),
            ).fetchone()
        return {"pages_read": row[0] or 0, "max_read_page": row[1] or 0}

    def pages_read_on(self, document_id, date):
        """Page numbers whose most recent read fell on the given local date
        ('YYYY-MM-DD'). Returns a sorted list."""
        with self._lock:
            rows = self._conn.execute(
                "SELECT page_number FROM page_reads "
                "WHERE document_id = ? AND date(last_read_at) = ? "
